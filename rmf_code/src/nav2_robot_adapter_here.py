...
class Nav2TfHandler:
    ...
    def __init__(self, robot_name, zenoh_session, tf_buffer, node):
        ...
        self.robot_name="mini1"#nav2_tb3
    ...
    def get_transform(self) -> TransformStamped | None:
        start_tf='base_footprint' #odom map base_footprint base_link
        end_tf='map'
        try:

            # TODO(ac): parameterize the frames for lookup
            transform = self.tf_buffer.lookup_transform(
                namespacify(start_tf, self.robot_name),
                namespacify(end_tf, self.robot_name),
                rclpy.time.Time()
            )
            return transform
        except Exception as err:
            self.node.get_logger().info(
                f'Unable to get transform between {start_tf} and {end_tf}: '
                f'{type(err)}: {err}'
