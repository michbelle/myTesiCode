...
\remove{In our case, we fused only the yaw angular velocity from the IMU while for the odometry we select only the velocity along the x and y.\\}
\alert{In our case, we fused the yaw angular velocity and the linear acceleration on x and y from the IMU with the velocity along the x and y  from the odometry in ENU (East North Up) coordinates.\\}
...
\end{listing}\\
\newpage
Since certain walls of the Cartographer map where not solid black\alert{, given by the uncertainty of the right position of them during mapping}, the conversion process introduced errors that caused these areas to be misinterpreted as free space. \alert{As a result, the planner generated paths that passed through them. To address this issue, the walls were manually corrected using GIMP, an image editing tool, by modifying the \texttt{.pgm} map file.\\ While, for our planner, that works considering that the unknown space is not usable for planning, we have to set the variable \texttt{track\_unknown\_space} in the global costmap node to  \texttt{true} (\texttt{false} by default) to obtain the best result from our planner.}
\remove{To address this issue, the walls were manually corrected using GIMP, an image editing tool, by modifying the \texttt{.pgm} map file.\\}
...
\alert{
	Hardware adaptation process from Rover Zero to Rover Mini -Rimuovo il capitolo?\\
}
\alert{
	Since we have another 2WD rover, called Jobot, still based on ROS 1. it was chosen to port also that robot to ROS 2 to best evaluate the use of Open-RMF.
}
\section{Jobot Description}
\alert{
Jobot is a 6 wheels with 2WD rover. It has a mini pc that collect data from a Rplidar  A2 laser sensor and acceleration data from a serial bus to a Arduino microprocessor connected to an IMU.\\The robot is the following one:\\
}
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{img/jobot_robot.jpg}
	\captionof{figure}{Jobot robot}
\end{figure}\\
\alert{
	\\The steps taken in action to use the code in ROS 2 is the following.
}
\section{What changes where made in the code}
\alert{
	The code use the serial ROS 1 library to communicate to the Arduino and retrive the data from the encoders and the imu data. This is not more available in ROS 2 and to avoid to rewrite the code we use some code porting available on the internet. In our case: 
	https://github.com/iwelch82/serial-ros2.git
	Then the data are used to publish and odometry pose estimation of the robot and also to publish the data of the imu using a secondary node. This node is updated to use a base node logic while the principal node that call it is wrote using the lifecycle structure. The logic is used to configure the node during on\_configure and if fail stop and clean everything when it is request of deactivate or cleanup. This structure allow to remotely control the passage through the states, for now and in our case it is going to be associated to an automatic manager used in nav2 the \texttt{lifecycle\_manager} in \texttt{nav2\_lifecycle\_manager}. This node check all the node that are connected to him and sequentialy configure all the node and then activate. If for some reason someone crashed or we want do kill a process also for the other nodes the procedure is communicated and a procedure of stop all nodes is activated. This node is only called in the launch file.
	Also the Jobot has a series of custom messages used internally, they where update using the ROS 2 standard.
	When it all configured then the node start to publish the robot state odometry but doesnt publish static trasform between the sensors, so they are manualy published using the \texttt{static\_transform\_publisher} provided by \texttt{tf2\_ros}.\\
}
\add{
	To avoid a lot of changes the software is build inside a docker container and all the applications are launched by it.\\
}
\alert{
	This and with the same software used for the navigation allow this platform to be an autonomous mobile robot.\\
}

\newpage
\section{Create a model for the simulation}
\alert{
For simulation purpose we need to create a model of it in the URDF format, that is going to be used by Gazebo to build a 3D model and generate all the information of the sensor that are used.
Since there are 2WD is going to add 2 cylinder as wheel that are controlled by this gazebo plugin \texttt{ignition::gazebo::systems::DiffDrive} that allow to receive the command of velocity and move the robot consecutively. Then we add the sensors types \texttt{imu} and \texttt{gpu\_lidar} to simulate the imu and the lidar setting their parameters as similar as the realistic sensors used. This modeling is similar to the editing of the Mini Rover from the Rover Robotics.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{is\_cleaning\_zone}: \remove{Used as initial position} \alert{Used to indicate that this waypoint belong to a cleaning zone }for the \texttt{Clean} task.
...
\alert{
	...
	\texttt{rmf\_jobot\_fleet\_adapter.launch.xml}\\
}
\newpage
...
\subsection{\alert{Deployment Template}}
Open-RMF offers a reference template\footnote{\href{https://github.com/open-rmf/rmf_deployment_template}{https://github.com/open-rmf/rmf\_deployment\_template}} for production deployment. Based on Kubernetes, it provides all the necessary information to deploy on a public server and to set up a secure connection.
...
\item \remove{We also encountered issues with the D* planner after converting the map to a PGM file. The algorithm treated unknown space as free space, which greatly increased computation time. The solution was to mark all unknown space as occupied, which resolved the problem.}

\item \alert{The D-star planner is not designed if encounter an obstacle during its path, it not update the costmap used internally at each request of the planner}
