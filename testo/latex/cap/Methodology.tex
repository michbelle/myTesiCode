...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{is\_cleaning\_zone}: Used as initial position for the \texttt{Clean} task.
...
\begin{itemize}
	...
	\item \texttt{lift\_superviso}: Publishes control requests for lifts using ROS communication structures.
...
\item We also encountered issues with the D* planner after converting the map to a PGM file. The algorithm treated unknown space as free space, which greatly increased computation time. The solution was to mark all unknown space as occupied, which resolved the problem.
