...
\section{changing in the code}
\alert{	We changed the strucure of hw the node work, i changed the strucure like a state machine used in the lifecycle node. this setup allow to control remotly the passages of the states of the robot allowing to obtain information when the robot has failed to configurate himself corretly. For now the passage is automatic controlled by an lifecycle manager node similar to the one used to control the navigation nodes.\\
similar changes from porting the navigation
}

\section{Create a model for the simulation}
\add{
For the simulation purpose we need to create a model of it in the URDF format, that is going to be used by Gazebo to build a 3D model and generate all the information of the sensor that are used.
For the creation of the model we are going to use:
ignition::gazebo::systems::DiffDrive
sensor name="imu" type="imu"
sensor name='gpu\_lidar' type='gpu\_lidar'
<plugin filename="libignition-gazebo-sensors-system.so" name="ignition::gazebo::systems::Sensors">
<render\_engine>ogre2</render\_engine>
</plugin>
Similar to the mini rover
}
\alert{in gazebo we take the input of the /cmd\_vel and publish data of odom to have a model of the robot avaible  in gazebo to simulation the robot also in gazebo}
